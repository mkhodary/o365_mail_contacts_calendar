        ...
        public async Task<List<MyContact>> GetContacts(int pageIndex, int pageSize)
        {
            ...
            foreach (var contact in results.CurrentPage)
            {
                myContactsList.Add(new MyContact
                {
                    ...
                    EmailAddress = contact.EmailAddresses != null && contact.EmailAddresses.Count > 0 ?
                                              contact.EmailAddresses[0].Address :
                                              string.Empty,
                    BusinessPhone = contact.BusinessPhones != null && contact.BusinessPhones.Count > 0 ? contact.BusinessPhones[0] : string.Empty,
                    HomePhone = contact.HomePhones != null && contact.HomePhones.Count > 0 ? contact.HomePhones[0] : string.Empty
                });
            }
